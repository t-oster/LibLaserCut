...
public class RasterBuilder implements Iterable<VectorCommand>, Iterator<VectorCommand>
{
  ...
  {
    lower_bound = image.leftMostNotEqual(y_position, skip_pixel_value);
    ...
    higher_bound = image.rightMostNotEqual(y_position, skip_pixel_value);
    return false;
  }

  private boolean initializeYAxisLine()
  {
    lower_bound = image.topMostNotEqual(x_position, skip_pixel_value);
    if ((lower_bound == -1) && ((transversal & NO_SKIP) == 0))
    {
      //This is a blank line. Keep stepping.
      x_position += dx;
    ...
    higher_bound = image.bottomMostNotEqual(x_position, skip_pixel_value);
    return false;
  }
  ...
  private void initGoingLeft()
  {
    ...
    if (inrange_y(y_position + dy))
    {
      ...
      end = Math.min(end, image.leftMostNotEqual(y_position + dy, skip_pixel_value));
    }
  ...
  private void initGoingTop()
  {
    ...
    if (inrange_x(x_position + dx))
    {
      ...
      end = Math.min(end, image.topMostNotEqual(x_position + dx, skip_pixel_value));
    }
  ...
  private void initGoingRight()
  {
    ...
    {
      //If the next line in the dy direction also has a further end. We stop there.
      end = Math.max(end, image.rightMostNotEqual(y_position + dy, skip_pixel_value));
    }
  ...
  private void initGoingBottom()
  {
    ...
    {
      //If the next line in the dx direction also has a further end. We stop there.
      end = Math.max(end, image.bottomMostNotEqual(x_position + dx, skip_pixel_value));
    }
  ...
  private void moveLeft()
  {
    ...
    x_position = image.nextColorChangeHeadingLeft(x_position, y_position, end);
    commitPosition();
  ...
  {
    getUpdatedPixelAtLocation();
    y_position = image.nextColorChangeHeadingTop(x_position, y_position, end);
    commitPosition();
  ...
  {
    getUpdatedPixelAtLocation();
    x_position = image.nextColorChangeHeadingRight(x_position, y_position, end);
    commitPosition();
  ...
  {
    getUpdatedPixelAtLocation();
    y_position = image.nextColorChangeHeadingBottom(x_position, y_position, end);
    commitPosition();
